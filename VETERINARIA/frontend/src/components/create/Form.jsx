...
export const Form = ({patient}) => {
    ...
    const registerPatient = async (data) => {
        ...
        */
        ...
        let url = `${import.meta.env.VITE_BACKEND_URL}/agenteSAC/registro`
        ...
        let response
        if (patient?._id) {
            url = `${import.meta.env.VITE_BACKEND_URL}/agenteSAC/actualizar/${patient._id}`
    ...
    useEffect(() => {
        ...
        <form onSubmit={handleSubmit(registerPatient)}>
            ...
            <fieldset className="border-2 border-blue-500 p-6 rounded-lg shadow-lg mt-10">
                <legend className="text-xl font-bold text-gray-700 bg-gray-200 px-4 py-1 rounded-md">
                    Información del Contrato
                </legend>
                ...
                <div>
                    ...
                    <input
                        ...
                        className="block w-full rounded-md border border-gray-300 py-1 px-2 text-gray-500 mb-5"
                        {...register("nombreContrato", { required: "El nombre del contrato es obligatorio" })}
                    />
                    {errors.nombreContrato && <p className="text-red-800">{errors.nombrecontrato.message}</p>}
                </div>

                {/* Imagen del contrato*/}
                <label className="mb-2 block text-sm font-semibold">Imagen del cotrato</label>
                <div className="flex gap-4 mb-2">
                    ...
                    <label className="flex items-center gap-2">
                        <input
                            type="radio"
                            value="ia"
                            {...register("imageOption",{ required: !patient && "El nombre del contrato es obligatorio"})}
                            disabled={patient}
                        />
                        Generar con IA
                    </label>
                ...
                <div>
                    <label className="mb-2 block text-sm font-semibold">Fecha de inicio contrato</label>
                    <input
                        type="date"
                        className="block w-full rounded-md border border-gray-300 py-1 px-2 text-gray-500 mb-5"
                ...
                {/* Especificaciones */}
                <div>
                    <label className="mb-2 block text-sm font-semibold">Especificaciones</label>
                    <textarea
                        placeholder="Ingresa los especificaciones"
                        className="block w-full rounded-md border border-gray-300 py-1 px-2 text-gray-500 mb-5"
                        {...register("especificacionesContrato", { required: "Especificaciones del contratro es obligatorio" })}
                    />
                    {errors.especificacionesContrato && <p className="text-red-800">{errors.especificacionesContrato.message}</p>}
                </div>
            </fieldset>
