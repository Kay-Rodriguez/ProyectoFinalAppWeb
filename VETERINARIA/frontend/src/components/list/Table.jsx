...
const Table = () => {
    ...
    const listPatients = async () => {
        const url = `${import.meta.env.VITE_BACKEND_URL}/agenteSACs`
    ...
    const deletePatient = async(id) => {
        const confirmDelete = confirm("Vas registrar la salida del agenteSAC, ¿Estás seguro?")
        if(confirmDelete){
            const url = `${import.meta.env.VITE_BACKEND_URL}/agenteSAC/eliminar/${id}`
            const storedUser = JSON.parse(localStorage.getItem("auth-token"))
            const options = {
                headers: {
                    "Content-Type": "application/json",
    ...
    return (
        ...
        <table className="w-full mt-5 table-auto shadow-lg bg-white">
            ...
            <thead className="bg-gray-800 text-slate-400">
                <tr>
                    {["N°", "Nombre Contrato", "Nombre propietario", "Email", "Celular", "Estado", "Acciones"].map((header) => (
